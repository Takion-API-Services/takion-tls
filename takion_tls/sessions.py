...
from .c.cffi import LibraryManager
from .models.cookies import cookiejar_from_dict, merge_cookies, extract_cookies_to_jar
from .exceptions import TLSException
from .models.structures import CaseInsensitiveDict
...
class Session:
    ...
    def __init__(
        ...
        if Session._library_manager is None:
            Session._library_manager = LibraryManager(Session.library_path)
            Session._library_manager.init_library()
            Session.clients = Session._library_manager.clients
        self.client_identifier = client if client is not None else self.clients.get_latest_chrome_version()
        ...
        self.headers = CaseInsensitiveDict(
            {
                "User-Agent": f"takion-tls/{__version__}",
    ...
    async def update_proxy(self, proxy: str) -> bool:
        ...
        Example:
            ...
            >>>
            >>> session = Session()
            >>> session.update_proxy("123:123")
            >>> session.proxies
            {'http': 'http://123:123', 'https': 'http://123:123'}
            >>> session.update_proxy("123:123:login:password")
            >>> session.proxies
        ...
        if 'http' in proxy:
            ...
            return update_session_proxy(self, proxy)
        return True
    ...
    async def execute_request(
        ...
        while 1:
            ...
            response = await loop.run_in_executor(None, Session._library_manager.library.request, dumps(request_payload).encode('utf-8'))
            ...
            # free the memory
            await loop.run_in_executor(None, Session._library_manager.library.freeMemory, response_object['id'].encode('utf-8'))

            # --- Response -------------------------------------------------------------------------------------------------
            # Error handling
            if response_object["status"] == 0:
                raise TLSException(response_object["body"])
